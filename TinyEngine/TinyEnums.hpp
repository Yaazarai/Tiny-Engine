#pragma once
#ifndef TINY_ENGINE_TINYENGINEENUMS
#define TINY_ENGINE_TINYENGINEENUMS
	...
	namespace TINY_ENGINE_NAMESPACE {
		...
		enum class TinyBufferType {
			/// @brief For passing mesh/triangle vertices for rendering to shaders.
			TYPE_VERTEX,
			/// @brief For indexing Vertex information in Vertex Buffers.
			TYPE_INDEX,
			/// @brief For passing uniform/shader variable data to shaders.
			TYPE_UNIFORM,
		...
		enum class TinyImageLayout {
			/// @brief Source image layout for transfering from A (source) to B (destination).
			LAYOUT_TRANSFER_SRC = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
			/// @brief Destination image layout for transfering from A (source) to B (destination).
		...
		/// @brief Image-Type for rendering operations.
		...
		enum class TinyBufferingMode {
			...
			MODE_TRIPLE = 3,
			/// @brief Quadruple-Buffering (pointless).
			MODE_QUADRUPLE = 4
		};

		/// @brief Command-Buffer submit-stage for specifying "when," we get be doing certain things during rendering: e.g. change image layouts.
		enum class TinyCmdBufferSubmitStage {
			/// @brief Pre-Render & Pre-Pipeline-Access & Pipeline-Acess submit stage.
			STAGE_BEGIN,
			/// @brief Post-Render & Post-Pipeline-Acess submit stage.
			STAGE_END,
			/// @brief No-Pipeline-Access submit stage (for layout transitions).
			STAGE_BEGIN_TO_END
		};
		...
		/// @brief GLFW Button Action (Press/Release) Bits.
		enum class TinyInputEvents {
			RELEASE = GLFW_RELEASE,
			PRESS = GLFW_PRESS,
		};
		
		/// @brief GLFW Modifer Keys Bits.
		enum class TinyModKey {
			KEY_NONE = 0,
			KEY_ALT = GLFW_MOD_ALT,
			KEY_CAPS_LOCK = GLFW_MOD_CAPS_LOCK,
			KEY_CONTROL = GLFW_MOD_CONTROL,
			KEY_NUMLOCK = GLFW_MOD_NUM_LOCK,
			KEY_SHIFT = GLFW_MOD_SHIFT,
			KEY_SUPER = GLFW_MOD_SUPER
		};
		
		/// @brief GLFW Keyboard Key bits.
		enum class TinyKeyboardButtons {
		...
		/// @brief GLFW Mouse Button Bits.
		enum class TinyMouseButtons {
		...
		/// @brief GLFW Gamepad Button Bits.
		enum class TinyGamepadButtons {
		...
		/// @brief GLFW Gamepad Axis Bits.
		enum class TinyGamepadAxis {
		...
		/// @brief GLFW Gamepad ID Bits.
		enum class TinyGamepads {
