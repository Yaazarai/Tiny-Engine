#ifndef TINY_ENGINE_SWAPCHAINHELPER
#define TINY_ENGINE_SWAPCHAINHELPER
	...
	namespace TINY_ENGINE_NAMESPACE {
		/* ✓ */ class TinySwapchain {
			...
			static inline TinySwapChainSupporter QuerySwapChainSupport(VkPhysicalDevice device, VkSurfaceKHR presentSurface) {
			...
			static inline VkSurfaceFormatKHR QuerySwapSurfaceFormat(TinySurfaceSupporter presentDetails, const std::vector<VkSurfaceFormatKHR>& availableFormats) {
			...
			static inline VkPresentModeKHR QuerySwapPresentMode(TinySurfaceSupporter presentDetails, const std::vector<VkPresentModeKHR>& availablePresentModes) {
			...
			static inline VkExtent2D QuerySwapExtent(TinyWindow& window, const VkSurfaceCapabilitiesKHR& capabilities) {
				int width, height;
			...
			static inline VkResult QueryNextSwapChainImage(TinyVkDevice& vkdevice, VkSwapchainKHR& swapChain, uint32_t &swapFrameIndex, VkFence imageAcquiredF = VK_NULL_HANDLE, VkSemaphore imageAcquiredS = VK_NULL_HANDLE) {
			...
			static inline VkResult CreateSwapChainImages(TinyVkDevice& vkdevice, TinyWindow& window, TinySurfaceSupporter& presentDetails, VkSwapchainKHR& swapChain, std::vector<TinyImage*>& swapChainImages) {
				TinySwapChainSupporter swapChainSupport = QuerySwapChainSupport(vkdevice.physicalDevice, vkdevice.presentSurface);
				...
				uint32_t imageCount = std::min(swapChainSupport.capabilities.maxImageCount, std::max(swapChainSupport.capabilities.minImageCount, static_cast<uint32_t>(TinyBufferingMode::MODE_DOUBLE)));
				...
				for(uint32_t i = 0; i < imageCount; i++) {
					swapChainImages[i] = new TinyImage(vkdevice, TinyImageType::TYPE_SWAPCHAIN, extent.width, extent.height, presentDetails.dataFormat, VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER, false, newSwapImages[i], VK_NULL_HANDLE, VK_NULL_HANDLE);
			...
			static inline void CreateSwapChainImageViews(TinyVkDevice& vkdevice, TinySurfaceSupporter& presentDetails, std::vector<TinyImage*>& swapChainImages) {
				TinySwapChainSupporter swapChainSupport = QuerySwapChainSupport(vkdevice.physicalDevice, vkdevice.presentSurface);
				VkSurfaceFormatKHR surfaceFormat = QuerySwapSurfaceFormat(presentDetails, swapChainSupport.formats);
			...
			static inline void WaitResetFences(TinyVkDevice& vkdevice, VkFence* fence) {
			...
			static inline void PushPresentMode(TinySurfaceSupporter& presentDetails, VkPresentModeKHR presentMode, std::atomic_bool& refreshFlag) {
