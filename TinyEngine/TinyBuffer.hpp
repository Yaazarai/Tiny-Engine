...
#define TINY_ENGINE_TINYBUFFER
	...
	namespace TINY_ENGINE_NAMESPACE {
		...
		public:
			...
			void StageBufferDataQueue(std::vector<std::tuple<TinyBuffer&, void*, VkDeviceSize, VkDeviceSize, VkDeviceSize>> buffers) {
				TinyBuffer& startBuffer = std::get<0>(buffers[0]);
				std::pair<VkCommandBuffer,int32_t> bufferIndexPair = BeginTransferCmd();

				for(std::tuple<TinyBuffer&, void*, VkDeviceSize, VkDeviceSize, VkDeviceSize> staging : buffers) {
					TinyBuffer& buffer = std::get<0>(staging);
			...
			void GetPipelineBarrierStages(TinyCmdBufferSubmitStage cmdBufferStage, VkPipelineStageFlags& srcStage, VkPipelineStageFlags& dstStage, VkAccessFlags& srcAccessMask, VkAccessFlags& dstAccessMask) {
				switch(cmdBufferStage) {
                    case TinyCmdBufferSubmitStage::STAGE_BEGIN:
                        switch(bufferType) {
                    ...
                    break;
                    case TinyCmdBufferSubmitStage::STAGE_END:
                        switch(bufferType) {
                            case TinyBufferType::TYPE_STAGING:
                            srcAccessMask = VK_ACCESS_NONE;
                            dstAccessMask = VK_ACCESS_NONE;
                    ...
                    break;
                    case TinyCmdBufferSubmitStage::STAGE_BEGIN_TO_END:
                        srcStage = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
                        dstStage = VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT;
                        srcAccessMask = VK_ACCESS_NONE;
                        dstAccessMask = VK_ACCESS_NONE;
			...
			template<typename... A>
			inline static TinyConstruct<TinyBuffer> Construct(TinyRenderContext& renderContext, TinyBufferType type, VkDeviceSize dataSize) {
				std::unique_ptr<TinyBuffer> object =
					std::make_unique<TinyBuffer>(renderContext, type, dataSize);
				return TinyConstruct<TinyBuffer>(object, object->Initialize());
			}

			/// @brief Constructor(...) + Initialize() with error result as combined TinyConstruct<Object,VkResult>.
			template<typename... A>
			inline static TinyConstruct<TinyBuffer> Construct(TinyRenderContext& renderContext, TinyBufferType type, void* data, VkDeviceSize dataSize, VkDeviceSize srcOffset, VkDeviceSize dstOffset) {
				std::unique_ptr<TinyBuffer> object =
					std::make_unique<TinyBuffer>(renderContext, dataSize, type);
				VkResult result = object->Initialize();
				object->StageBufferData(data, dataSize, srcOffset, dstOffset);
				return TinyConstruct<TinyBuffer>(object, result);
			}
